...
def animate(i):
    ...
    ax.set_title(f"U: Time Step: {i}, Time: {i * delta_t:.4f} s, P: {i/N_TIME_STEPS * 100:.2f}%")
    # ax.set_ylim(-100, 100)
...
# Create an animation
ani = animation.FuncAnimation(fig, animate, frames=range(0, int(N_TIME_STEPS), SKIP_FRAMES), interval=150)
